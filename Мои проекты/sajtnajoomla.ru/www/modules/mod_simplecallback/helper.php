...
class modSimpleCallbackHelper
{
    public static function getAjax()
    {
        ...
        $recipients_from = $params->get('simplecallback_emails_from');
        ...
        // if(!empty($page_url) && $params->get('simplecallback_page_url') == 1) :  $body .= "\n" . "Title: " . $page_title; endif;
        // $redirect_url_title = '';
        // if($params->get('simplacallback_redirect_url_title') == 1) :  $redirect_url_title = $page_url; endif;
        // if(!empty($redirect_url_title) && $params->get('simplacallback_redirect_url_title') == 1) :  $body .= "\n" . "URL: " . $redirect_url_title; endif;
        ...
        if ($recipients_from == 1) {
            $mail->setSender($emailclient);
        }
        else {
        ...
        if (true == $sent) {
           ...
           if ($pozvonim_enabled === '1' && !empty($pozvonim_key)) {
		    ...
		    if ($vk_enabled === '1' && !empty($vk_access_token)) {
                ...
                vkpush('wall.post', array('owner_id' => '-'.$vk_group_id,  'from_group' => $vk_from_group, 'message' => $vk_message,  'access_token' => $vk_access_token, 'v' => '5.84'));

                }

                if ($vk_board_comment === '1') {

                vkpush("board.createComment", array("group_id" => $vk_group_id, "topic_id" => $vk_topic_id, "message" => $vk_message,  "from_group" => $vk_from_group,  "access_token" => $vk_access_token, 'v' => '5.84'));
