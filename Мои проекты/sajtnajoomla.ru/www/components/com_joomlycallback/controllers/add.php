...
class JoomlycallbackControllerAdd extends JoomlycallbackController
{
	...
	{	
		...
		$empty_fields = !empty($data["times"]) ? 1 : 0;
		
		if (isset($params["captcha"]) && isset($params["admin_mail"]) && $data['callback_module_hash'] == JUserHelper::getCryptedPassword($url->toString() . $params["admin_mail"]) && $empty_fields == 0)
		{
			...
			if  ($params["captcha"] == 1)
			{
				...
				curl_setopt($ch, CURLOPT_POSTFIELDS, array(
					...
					'remoteip' => $data['ip'], 
					'response' => ($params["captcha_size"] == "invisible") ? $data["module_token"] : $data['g-recaptcha-response']
				));
		...
		} else 
		{
			setcookie('callback_captcha', null, -1,'/',null);
			setcookie('callback-sending-alert',$mod,time()+60,'/',null);
			setcookie('callback-alert-type',"defense",time()+60,'/',null);
			$app->redirect(JRoute::_($url_redirect, false));	
		}
	}
}
