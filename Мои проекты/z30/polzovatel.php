...
$sel="SELECT COUNT(*) FROM `polzovatel`";
$res_count = mysqli_query($connection, $sel);
$row = mysqli_fetch_row($res_count);
$total = $row[0];
$str_pag = ceil($total / $limit);
$query = mysqli_query($connection, "SELECT * FROM `polzovatel` ORDER BY id, name, password LIMIT $number, $limit");
